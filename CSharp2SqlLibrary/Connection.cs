...
namespace CSharp2SqlLibrary {
    public class Connection {
        ...
        public SqlConnection sqlConnection { get; set; } = null;   //2. now we create an actual connection object. It's a property.// class name is also "Connection".  // this is the actual connection?//underscore in line above:  a dev shortcut to indicate a PROPERTY. 

        public void Open() {
            this.sqlConnection.Open();  //Greg typically doesn't handle the possible exceptions.  Let it blow.  Almost always, the connection string is the problem. 
            //now check whether you've actually opened a connection:
            if (this.sqlConnection.State != System.Data.ConnectionState.Open) { //"Open is one of the states.  hover over connectionstate
        ...
        //public SqlConnection sqlConnection { get; set; } = null; // class name is also "Connection"
        ...
        //    this.sqlConnection = new SqlConnection(connStr); //we have created a connection object now
